...
strategyReturnsL <- function(data, thres){
  dat <- data[data$r_on<thres,]
...
ReturnsL$rowname <- rownames(ReturnsL)

nSLL <- NShortOrLong(dat,thres)
nSLL <- data.frame(nSLL)
nSLL$rowname <- rownames(nSLL)


fwrite(ReturnsL,"./Data/AReturnsON_L.csv")
fwrite(nSLL,"./Data/AnSLON_L.csv")
